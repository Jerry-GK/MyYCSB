...
public class RocksDBClient extends DB {
  ...
  private RocksDB initRocksDBWithOptionsFile() throws IOException, RocksDBException {
    ...
    final boolean enable_blob = false;
    ...
    // Configure blob options for each column family
    if (enable_blob) {
      for(ColumnFamilyDescriptor cfDesc : cfDescriptors) {
        ColumnFamilyOptions cfOptions = cfDesc.getOptions();
        cfOptions.setEnableBlobFiles(true)
                .setMinBlobSize(512)    // Values larger than 512B will be stored as blob
                .setBlobFileSize(64 * 1024 * 1024)  // Each blob file is 64MB  
                .setBlobCompressionType(CompressionType.LZ4_COMPRESSION)
                .setBlobGarbageCollectionAgeCutoff(0.25)  // Files with 25% garbage will be GC
                .setBlobGarbageCollectionForceThreshold(0.75);  // Must GC if 75% is garbage
      }
    }
  ...
  public Status delete(final String table, final String key) {
    try {
      ...
      // rocksDb.delete(cf, writeOptions, key.getBytes(UTF_8));
